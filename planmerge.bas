Option Explicit
Public lstImport As New Collection
Public lstExport As New Collection
Public strTemp As String
Public strFname As String
Public strExport As String
Public strSaveAs As String
Public varFiles As Variant
Sub WeldPlanMerger_v14()
Form.Show
...
Set wbTemp = Workbooks.Open("C:\WeldPlanMergeTemplate\WeldPlanExcelUploadTemplate.xlsx", False, False)
'//range 개체 선언

'// 각 WeldPlan 엑셀 파일의 Column을 Range 개체로 선언

Dim rngJoint As Range '// JOINT_NO

Dim rngSpool As Range '// SPOOL_NO

Dim rngSpec As Range '// SPEC

Dim rngMatl As Range '//RAW_MATERIAL

Dim rngSize As Range '//SIZE

Dim rngSch As Range '//SCHEDULE

Dim rngType As Range '//WELD_TYPE

Dim rngSF As Range '// SHOP_FIELD

Dim rngConIso As Range '//CON_ISONO

Dim rngConSpool As Range '//CON_SPOOL

Dim rngCopy As Range '// Range Copy용
...
Else
    If optSet = "Default" Then '// Setting이 "Default" 일 경우
        ...
        Application.ScreenUpdating = False
            ...
            Set rngJoint = wb.Sheets(1).Range(Range("A2"), Range("A" & Format(NoRow)))
        
            Set rngSpool = wb.Sheets(1).Range(Range("B2"), Range("B" & Format(NoRow)))
    
            Set rngSpec = wb.Sheets(1).Range(Range("C2"), Range("C" & Format(NoRow)))
    
            Set rngMatl = wb.Sheets(1).Range(Range("D2"), Range("D" & Format(NoRow)))
    
            Set rngSize = wb.Sheets(1).Range(Range("E2"), Range("E" & Format(NoRow)))
    
            Set rngSch = wb.Sheets(1).Range(Range("F2"), Range("F" & Format(NoRow)))
    
            Set rngType = wb.Sheets(1).Range(Range("G2"), Range("G" & Format(NoRow)))
    
            Set rngSF = wb.Sheets(1).Range(Range("H2"), Range("H" & Format(NoRow)))
    
            Set rngConIso = wb.Sheets(1).Range(Range("L2"), Range("L" & Format(NoRow)))
    
            Set rngConSpool = wb.Sheets(1).Range(Range("M2"), Range("M" & Format(NoRow)))
